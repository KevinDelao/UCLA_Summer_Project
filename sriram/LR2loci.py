...
def shap_acc_LR_set(samples_n, loci_m, beta_g, e_noise, number_trials, confidence=0.95):
    ...
    while counter != number_trials:
        ...
        G, loci = trait_simulation_twoloci_inter(samples_n, loci_m, beta_g, e_noise, 2, 0)
        shap_values_holder, x_train = shap_LR_kernel_train(G)
        #max_holder = max_mean_feature(shap_values_holder)
        predicted_loci = mean_shap_values(shap_values_holder)
        num_correct = accuracy_count(loci, predicted_loci)
        shap_values_SNP .append(num_correct)
